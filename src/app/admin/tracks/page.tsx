    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/tracks/new" className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90">+ Add Track</Link>
        </div>
        {tracks.length === 0 ? (
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-6 py-16 text-center">
            ...
            <Link href="/admin/tracks/new" className="mt-3 inline-block text-sm text-[var(--accent)] hover:underline">Add your first track</Link>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {tracks.map((t) => (
              <div key={t.id} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 space-y-3 hover:border-[var(--accent)]/40 transition-colors">
                <div>
                  <Link href={`/admin/tracks/${t.id}`} className="font-semibold text-white hover:text-[var(--accent)] transition-colors">{t.name}</Link>
