...
export default function NewTrackPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) setError(result.error); else router.push("/admin/tracks");
      ...
      <main className="mx-auto max-w-xl px-4 sm:px-6 py-10">
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={pending} className="flex-1 rounded-lg bg-[var(--accent)] py-2.5 text-sm font-semibold text-white disabled:opacity-50 hover:opacity-90">{pending ? "Saving…" : "Add Track"}</button>
