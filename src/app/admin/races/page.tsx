    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/races/new" className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-black hover:opacity-90">+ New Race</Link>
        </div>
        {races.length === 0 ? (
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-6 py-16 text-center">
            ...
            <Link href="/admin/races/new" className="mt-3 inline-block text-sm text-[var(--accent)] hover:underline">Create your first race</Link>
          </div>
        ) : (
              ...
              <tbody>
                {races.map((r) => (
                  <tr key={r.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-raised)] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/races/${r.id}`} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--muted)] hover:text-white hover:border-[var(--accent)] transition-colors">View</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
