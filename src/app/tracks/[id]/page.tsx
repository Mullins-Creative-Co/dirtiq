  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-10 space-y-8">
        ...
        {/* Header */}
        <div>
          <Link href="/tracks" className="text-xs text-[var(--muted)] hover:text-white transition-colors">← Tracks</Link>
                  ...
                  {races.map((r) => (
                    <tr key={r.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-raised)] transition-colors">
                      <td className="px-4 py-3">
                        <Link href={`/races/${r.id}`} className="font-medium text-white hover:text-[var(--accent)] transition-colors">{r.name}</Link>
                      </td>
